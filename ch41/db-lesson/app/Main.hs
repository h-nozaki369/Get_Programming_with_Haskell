...
checkout userId toolId = withConn "tools.db" $ \conn -> do
    execute conn
        "INSERT INTO checkout (user_id,tool_id) VALUES (?,?)" (userId, toolId)
...
printAvailable = printToolQuery $
    mconcat [ "select * from tools "
            , "where id not in "
            , "(select tool_id from checkout);" ]

printCheckout :: IO ()
printCheckout = printToolQuery $
    mconcat [ "select * from tools "
            , "where id in "
            , "(select tool_id from checkout);" ]
...
checkin toolId = withConn "tools.db" $ \conn -> do
    execute conn "DELETE FROM checkout WHERE tool_id = (?);" (Only toolId)
...
promptAndAddTool = do
    ...
    toolName <- getLine
    print "Enter a description of new tool"
    toolDesc <- getLine
...
-- Q41-2
    ...
    | command == "out" = printCheckout >> main
